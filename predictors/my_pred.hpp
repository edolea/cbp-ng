...
// Step 3 — TAGE-N: a bimodal "base" plus NUMG tagged tables indexed by
// geometrically-increasing global-history lengths (folded down to the index /
// tag width by geometric_folds). The prediction comes from the LONGEST-history
// table whose tag matches; on a misprediction we allocate a new entry in a
// table LONGER than the current provider. No chooser anywhere.
//
// Table ordering follows geometric_folds: table 0 has the LONGEST history,
// table NUMG-1 the shortest. So "longest match wins" == lowest matching index.
...
struct my_pred_tage : predictor {
    ...
    static constexpr u64 CTRBITS = 3;                  // direction counter, MSB = taken
    ...
    // --- NUMG tagged tables, indexed by PC ^ folded history of length HLEN[i] ---
    ram<val<EW>, GTABLE> gt[NUMG];
    // folded global history: 2 folds per length (fold<0>=index, fold<1>=tag)
    geometric_folds<NUMG, MINHIST, MAXHIST, LOGG, TAGBITS> gfolds;

    // values saved at predict time, reused at update time
    ...
    val<1> predict1(val<64> inst_pc) {
        ...
        return bctr >> 1;                       // P1 = base bimodal (cheap, fast)
    }
